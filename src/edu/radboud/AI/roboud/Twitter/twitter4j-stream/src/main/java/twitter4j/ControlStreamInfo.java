...
import static ParseUtil.getBoolean;
import static ParseUtil.getRawString;
...
public final class ControlStreamInfo implements Serializable {
    ...
    /*package*/ ControlStreamInfo(StreamController controller, JSONObject json) throws TwitterException {
        try {
            ...
            includeFollowingsActivity = ParseUtil.getBoolean("include_followings_activity", info);
            includeUserChanges = ParseUtil.getBoolean("include_user_changes", info);
            replies = ParseUtil.getRawString("replies", info);
            with = ParseUtil.getRawString("with", info);
