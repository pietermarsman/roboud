...
public class UserStreamTest extends TwitterTestBase implements UserStreamListener {
    ...
    public void testUserStreamEventTypes() throws Exception {
        ...
        stream.next(this);
        waitForStatus("follow", TwitterMethod.CREATE_FRIENDSHIP);
        ...
        ex = null;
        stream.next(this);
        waitForStatus("delete direct message", TwitterMethod.DESTROY_DIRECT_MESSAGE);
...
//        waitForStatus("unknown", "unknown");
    ...
    public void testUserStream() throws Exception {
        ...
        waitForStatus("sentDirectMessage", TwitterMethod.SEND_DIRECT_MESSAGE);
...
//        waitForStatus("destroyedDirectMessage");
        ...
        waitForStatus("onStatus", TwitterMethod.UPDATE_STATUS);

        twitter1.retweetStatus(status.getId());
        waitForStatus("onStatus", TwitterMethod.UPDATE_STATUS);

        twitter1.createFavorite(status.getId());
        waitForStatus("createdFavorite", TwitterMethod.CREATE_FAVORITE);

        twitter1.destroyFavorite(status.getId());
        waitForStatus("destroyedFavorite", TwitterMethod.DESTROY_FAVORITE);

        // unfollow twit4j
        twitter1.destroyFriendship(id2.id);
        waitForStatus("destroyedFriendship", TwitterMethod.DESTROY_FRIENDSHIP);

        // follow twit4j
        twitter1.createFriendship(id2.id);
        waitForStatus("createdFriendship", TwitterMethod.CREATE_FRIENDSHIP);

        status = twitter1.updateStatus("somerandometext " + new Date());
        waitForStatus("updatedStatus", TwitterMethod.UPDATE_STATUS);

        twitter1.destroyStatus(status.getId());
        waitForStatus("destroyedStatus", TwitterMethod.DESTROY_STATUS);

        // block twit4j2
        twitter1.createBlock(id2.id);
        waitForStatus("createdBlock", TwitterMethod.CREATE_BLOCK);

        // unblock twit4j2
        twitter1.destroyBlock(id2.id);
        waitForStatus("destroyedBlock", TwitterMethod.DESTROY_BLOCK);

        twitter1.updateProfile(null, null, new Date().toString(), null);
        waitForStatus("updateProfile", TwitterMethod.UPDATE_PROFILE);

        UserList list = twitter1.createUserList("test", true, "desctription");
        waitForStatus("createdUserList", TwitterMethod.CREATE_USER_LIST);

        list = twitter1.updateUserList(list.getId(), "test2", true, "description2");
        waitForStatus("updatedUserList", TwitterMethod.UPDATE_USER_LIST);

        twitter1.createUserListMember(list.getId(), id2.id);
        waitForStatus("addedListMember", TwitterMethod.CREATE_LIST_MEMBER);

        twitter2.createUserListSubscription(list.getId());
        waitForStatus("createdUserListSubscription", TwitterMethod.SUBSCRIBE_LIST);

        twitter1.destroyUserListMember(list.getId(), id2.id);
        waitForStatus("deletedUserListMember", TwitterMethod.DESTROY_LIST_MEMBER);

        twitter2.destroyUserListSubscription(list.getId());
        waitForStatus("destroiedUserListSubscription", TwitterMethod.UNSUBSCRIBE_LIST);

        twitter1.destroyUserList(list.getId());
        waitForStatus("destroyedUserList", TwitterMethod.DESTROY_USER_LIST);
...
//        assertReceived("onDeletionNotice-directmessage", TwitterMethod.DESTROY_DIRECT_MESSAGE);
        ...
        for (Object[] event : this.received) {
            if (TwitterMethod.UPDATE_STATUS.equals(event[0])) {
    ...
    public void onStatus(Status status) {
        ...
        received.add(new Object[]{TwitterMethod.UPDATE_STATUS, status});
