...
public class AsyncTwitterTest extends TwitterTestBase implements TwitterListener {
    ...
    public void testAccountMethods() throws Exception {
        ...
        waitForResponse();
        Assert.assertEquals("FF0000", user.getProfileBackgroundColor());
        Assert.assertEquals("FF00FF", user.getProfileTextColor());
        Assert.assertEquals("00FFFF", user.getProfileLinkColor());
        Assert.assertEquals("00FF00", user.getProfileSidebarFillColor());
        Assert.assertEquals("FF00FF", user.getProfileSidebarBorderColor());
        async1.updateProfileColors("f0f", "f00", "f0f", "0ff", "0f0");
        waitForResponse();
        Assert.assertEquals("FF00FF", user.getProfileBackgroundColor());
        Assert.assertEquals("FF0000", user.getProfileTextColor());
        Assert.assertEquals("FF00FF", user.getProfileLinkColor());
        Assert.assertEquals("00FFFF", user.getProfileSidebarFillColor());
        Assert.assertEquals("00FF00", user.getProfileSidebarBorderColor());
...
//        assertEquals(403, te.getStatusCode());
        ...
        // now it returns 403
        Assert.assertEquals(403, te.getStatusCode());
        Assert.assertEquals(108, te.getErrorCode());
    ...
    public void testRateLimitStatus() throws Exception {
        ...
        Assert.assertTrue(1 < status.getLimit());
        Assert.assertTrue(1 < status.getRemaining());
    }
    ...
    public void testAppOnlyAuthWithBuildingConf2() throws Exception {
        ...
        TwitterListener listener = new TwitterAdapter() {
            ...
            public void gotRateLimitStatus(Map<String, RateLimitStatus> rateLimitStatus) {
                ...
                Assert.assertNotNull(searchTweetsRateLimit);
                Assert.assertEquals(searchTweetsRateLimit.getLimit(), 450);
                notifyResponse();
            }
            ...
            public void onException(TwitterException ex, TwitterMethod method) {
                ...
                Assert.assertEquals(220, ex.getErrorCode());
                Assert.assertEquals("Your credentials do not allow access to this resource", ex.getErrorMessage());
                notifyResponse();
            }
